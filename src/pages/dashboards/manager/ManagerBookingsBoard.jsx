...
const ManagerBookingsBoard = () => {
  ...
  // Fetch deliveries and drivers from API
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        ...
        // Fetch drivers
        const driversResponse = await axiosInstance.get('/api/admin/users?role=DRIVER');

        if (deliveriesResponse.data.success) {
          const mappedDeliveries = deliveriesResponse.data.data.map((delivery) => ({
            id: delivery.id,
        ...
        if (driversResponse.data.success) {
          const mappedDrivers = driversResponse.data.data.map((driver) => ({
            id: driver.id,
