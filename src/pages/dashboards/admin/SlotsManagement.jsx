...
const SlotsManagement = () => {
    ...
    const [capacityAmount, setCapacityAmount] = useState(5);
    const [capacityMethod, setCapacityMethod] = useState('increase');
    ...
    const handleUpdateCapacity = async () => {
        try {
            ...
            const response = await axiosInstance.put(
                `/api/admin/slots/${selectedSlot.id}/capacity`,
                {
                    method: capacityMethod,
                    value: parseInt(capacityAmount),
                }
            );
    ...
    const stats = {
        ...
        totalAvailable: slots.reduce(
            (sum, slot) => sum + (slot.maxCapacity - slot.booked),
            0
        ),
    };
            ...
            >
                ...
                <div className="mb-4 grid grid-cols-3 gap-2 sm:gap-4">
                    <div className="rounded-lg bg-white p-2 text-center shadow-sm sm:p-3">
                        ...
                        <p className="text-lg font-bold text-teal-600 sm:text-2xl">
                            {slot.maxCapacity}
                        </p>
                    </div>
                    <div className="rounded-lg bg-white p-2 text-center shadow-sm sm:p-3">
                        ...
                        <div className="block lg:hidden">
                            <div className="space-y-4">
                                {paginatedSlots.map((slot) => {
                                    ...
                                    const utilization = ((slot.booked / slot.maxCapacity) * 100).toFixed(0);
                                    return (
                                        <div key={slot.id} className="overflow-hidden rounded-lg bg-white shadow-sm border border-gray-200">
                                            {/* Card Header */}
                                            <div className="flex items-center justify-between bg-linear-to-r from-teal-600 to-teal-500 px-4 py-3">
                                                ...
                                                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${slot.timeSlot === 'AM'
                                            ...
                                            <div className="p-4 space-y-3">
                                                ...
                                                <div className="flex items-center justify-between pt-2">
                                                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${slot.isFull
                                    ...
                                    <div className="overflow-x-auto">
                                        <table className="min-w-full divide-y divide-gray-200">
                                            <thead className="bg-gray-50">
                                                <tr>
                                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 sm:px-6">
                                                        Date
                                                    </th>
                                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 sm:px-6">
                                                        Time
                                                    </th>
                                                    <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-700 sm:px-6">
                                                        Capacity
                                                    </th>
                                                    <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-700 sm:px-6">
                                                        Booked
                                                ...
                                                {paginatedSlots.map((slot) => {
                                                    ...
                                                    return (
                                                        <tr key={slot.id} className="transition-colors hover:bg-gray-50">
                                                            ...
                                                            <td className="whitespace-nowrap px-4 py-4 text-center text-sm font-semibold text-gray-900 sm:px-6">
                                                                {slot.maxCapacity}
                                                            </td>
                                                            <td className="whitespace-nowrap px-4 py-4 text-center text-sm font-semibold text-orange-600 sm:px-6">
                                                                {slot.booked}
                                                            </td>
                                                            <td className="whitespace-nowrap px-4 py-4 text-center text-sm font-semibold text-green-600 sm:px-6">
                                                                {available}
                                                            </td>
                                                            <td className="whitespace-nowrap px-4 py-4 sm:px-6">
                                                            ...
                                                            <td className="whitespace-nowrap px-4 py-4 text-center sm:px-6">
                                                                <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${slot.isFull
                                                                    ? 'bg-red-100 text-red-800'
                                                                    : 'bg-green-100 text-green-800'
                                                                    }`}>
                                                                    {slot.isFull ? 'FULL' : 'AVAILABLE'}
                        ...
                        <div className="space-y-4 p-4 sm:p-6">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setNewSlotData({ ...newSlotData, date: e.target.value })
                                    }
                                    className="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setNewSlotData({ ...newSlotData, timeSlot: e.target.value })
                                    }
                                    className="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
                                >
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setNewSlotData({ ...newSlotData, maxCapacity: e.target.value })
                                    }
                                    className="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
                                />
                            </div>
                    ...
                    <div className="w-full max-w-md rounded-lg bg-white shadow-xl">
                        ...
                        <div className="space-y-4 p-4 sm:p-6">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <button
                                        onClick={() => setCapacityMethod('increase')}
                                        className={`flex items-center justify-center space-x-2 rounded-lg border-2 px-4 py-3 text-sm font-medium transition-colors ${capacityMethod === 'increase'
                                    ...
                                    <button
                                        onClick={() => setCapacityMethod('decrease')}
                                        className={`flex items-center justify-center space-x-2 rounded-lg border-2 px-4 py-3 text-sm font-medium transition-colors ${capacityMethod === 'decrease'
                            ...
                            <div className="rounded-lg border-2 border-teal-200 bg-teal-50 p-4">
                                <p className="text-sm font-medium text-teal-900">New Capacity:</p>
                                <p className="text-2xl font-bold text-teal-600">
                                    {capacityMethod === 'increase'
                                        ? selectedSlot.maxCapacity + parseInt(capacityAmount || 0)
                                        : Math.max(
                                            selectedSlot.booked,
                                            selectedSlot.maxCapacity - parseInt(capacityAmount || 0)
                                        )}
                        ...
                        <div className="flex items-center justify-end space-x-3 border-t border-gray-200 p-4 sm:p-6">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className={`rounded-lg px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 ${capacityMethod === 'increase' ? 'bg-green-600' : 'bg-red-600'
                                    }`}
                            >
